...
class BafflePlanner(ScriptedLoadableModule):
  ...
  def __init__(self, parent):
    ...
    self.parent.title = _("Baffle Planner")
    self.parent.categories = [_("Cardiac")]
    self.parent.dependencies = []
    self.parent.contributors = ["mik adds"]
...
class BafflePlannerWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
  ...
  def onFlattenProgress(self, text):
    self.ui.flattenButton.text = _("Flattening in progress... ") + text
    slicer.app.processEvents()

  def onNurbsConvertCancelButton(self):
    if not hasattr(slicer.modules, 'nurbsfitting'):
      slicer.util.messageBox(_("The NURBS conversion requires the NurbsFitting module in the SlicerHeartPrivate extension. Install SlicerHeartPrivate and restart Slicer."))
      return

  def onFlattenButton(self):
    try:
      self.ui.flattenButton.enabled = False
      self.ui.flattenButton.text = _("Flattening in progress...")
      self.logic.flattenOutputBaffleModel(self.onFlattenProgress)
    except Exception as e:
      import traceback
      traceback.print_exc()
      slicer.util.errorDisplay("Error flattening baffle image: "+str(e))
    self.ui.flattenButton.text = _("Flatten")
