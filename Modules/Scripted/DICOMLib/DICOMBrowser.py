...
class SlicerDICOMBrowser(VTKObservationMixin, qt.QWidget):
    ...
    def setup(self, showPreview=False):
        ...
        self.setWindowTitle(translate("DICOMLib.DICOMBrowser", 'DICOM Browser'))
        ...
        self.uncheckAllButton = qt.QPushButton(translate("DICOMLib.DICOMBrowser", 'Uncheck All'))
        ...
        self.examineButton = qt.QPushButton(translate("DICOMLib.DICOMBrowser", 'Examine'))
        ...
        self.loadButton = qt.QPushButton(translate("DICOMLib.DICOMBrowser", 'Load'))
        self.loadButton.setSizePolicy(qt.QSizePolicy.Expanding, qt.QSizePolicy.Fixed)
        self.loadButton.toolTip = translate("DICOMLib.DICOMBrowser", 'Load selected items into the scene')
        self.actionButtonLayout.addWidget(self.loadButton)
        self.loadButton.connect('clicked()', self.loadCheckedLoadables)

        self.actionButtonLayout.addStretch(0.05)

        self.advancedViewButton = qt.QCheckBox(translate("DICOMLib.DICOMBrowser", 'Advanced'))
    ...
    def promptForExtensions(self):
        ...
        if len(extensionsToOffer) != 0:
            ...
            if len(displayedTypeDescriptions) == 1:
                message += translate("DICOMLib.DICOMBrowser", "The following data type is in your database:")
            else:
                message += translate("DICOMLib.DICOMBrowser", "The following data types are in your database:")
            message += " <b>" + ", ".join(displayedTypeDescriptions) + "</b>."

            message += "<p><p>"
            if len(displayedExtensionNames) == 1:
                message += translate("DICOMLib.DICOMBrowser", "The following extension is not installed, but may help you work with this data:")
            else:
                message += translate("DICOMLib.DICOMBrowser", "The following extensions are not installed, but may help you work with this data:")
            message += " <b>" + ", ".join(displayedExtensionNames) + "</b>."

            message += "<p><p>"
            message += translate("DICOMLib.DICOMBrowser", "You can install extensions using the Extensions Manager option from the View menu.")
            slicer.util.infoDisplay(message, parent=self, windowTitle=translate("DICOMLib.DICOMBrowser", 'DICOM'))
    ...
    def getLoadablesFromFileLists(self, fileLists):
        ...
        if missingFileCount > 0:
            messages.append(
                translate("DICOMLib.DICOMBrowser", "Warning: {missing_file_count} of {total_file_count} selected files listed in the database cannot be found on disk.").format(
        ...
        if missingFileCount < allFileCount:
            ...
            def progressCallback(progressDialog, progressLabel, progressValue):
                progressDialog.labelText = '\n' + translate("DICOMLib.DICOMBrowser", "Checking {what}").format(what=progressLabel)
        ...
        if messages:
            slicer.util.warningDisplay(
                translate("DICOMLib.DICOMBrowser", "Warning: {messages}").format(messages=' '.join(messages))
                + "\n\n" + translate("DICOMLib.DICOMBrowser", "See python console for error message."),
                windowTitle=translate("DICOMLib.DICOMBrowser", "DICOM"), parent=self)
    ...
    def proceedWithReferencedLoadablesSelection(self):
        ...
        def progressCallback(progressDialog, progressLabel, progressValue):
            progressDialog.labelText = '\n' + translate("DICOMLib.DICOMBrowser", "Loading {what}").format(what=progressLabel)
            slicer.app.processEvents()
            progressDialog.setValue(progressValue)
            slicer.app.processEvents()
            cancelled = progressDialog.wasCanceled
        ...
        if messages:
            slicer.util.warningDisplay('\n'.join(messages), windowTitle=translate("DICOMLib.DICOMBrowser", 'DICOM loading'))
    ...
    def warnUserIfLoadableWarningsAndProceed(self):
        ...
        for plugin in self.loadablesByPlugin:
            for loadable in self.loadablesByPlugin[plugin]:
                if loadable.selected and loadable.warning != "":
                    ...
                    logging.warning(translate("DICOMLib.DICOMBrowser", 'Warning in DICOM plugin {load_type} when examining loadable {name}: {message}').format(
        ...
        if warningsInSelectedLoadables:
            warning = translate("DICOMLib.DICOMBrowser", "Warnings detected during load.  Examine data in Advanced mode for details.  Load anyway?")
...
class DICOMReferencesDialog(qt.QMessageBox):
    ...
    WINDOW_TITLE = translate("DICOMLib.DICOMBrowser", "Referenced datasets found")
    WINDOW_TEXT = translate("DICOMLib.DICOMBrowser", "The loaded DICOM objects contain references to other datasets you did not select for loading."
    ...
    def setup(self):
        ...
        self.rememberChoiceAndStopAskingCheckbox = qt.QCheckBox(translate("DICOMLib.DICOMBrowser", 'Remember choice and stop asking'))
        self.rememberChoiceAndStopAskingCheckbox.toolTip = translate("DICOMLib.DICOMBrowser", 'Can be changed later in Application Settings / DICOM')
    ...
    def _addLoadableCheckboxes(self):
        self.checkBoxGroupBox = qt.QGroupBox(translate("DICOMLib.DICOMBrowser", "References"))
...
class DICOMLoadableTable(qt.QTableWidget):
    ...
    def configure(self):
        ...
        self.setHorizontalHeaderLabels([translate("DICOMLib.DICOMBrowser", 'DICOM Data'), translate("DICOMLib.DICOMBrowser", 'Reader'), translate("DICOMLib.DICOMBrowser", 'Warnings')])
