                                                                                ...
                                                                                double delaunayAlpha, bool smoothing, bool forceConvex)
{
  if (inputPoints == NULL)
  {
    vtkGenericWarningMacro("Input points are null. No model generated."); // ���inputPointsΪ�գ����������Ϣ
    return false;                                                         // ����false��ʾʧ��
  }

  if (outputPolyData == NULL)
  {
    vtkGenericWarningMacro("Output poly data is null. No model generated."); // ���outputPolyDataΪ�գ����������Ϣ
    return false;                                                            // ����false��ʾʧ��
  }

  int numberOfPoints = inputPoints->GetNumberOfPoints(); // ��ȡ����������
  if (numberOfPoints == 0)
  {
    // No markup points, the output should be empty // ���û�б�ǵ㣬���ӦΪ��
    return true; // ����true��ʾ�ɹ���û������ģ��
  }

  vtkSmartPointer<vtkCellArray> inputCellArray = vtkSmartPointer<vtkCellArray>::New();
  inputCellArray->InsertNextCell(numberOfPoints); // ����һ���������е�ĵ�Ԫ
  for (int i = 0; i < numberOfPoints; i++)
  {
    inputCellArray->InsertCellPoint(i); // ��ÿ������뵥Ԫ��
  }

  vtkSmartPointer<vtkPolyData> inputPolyData = vtkSmartPointer<vtkPolyData>::New(); // ����һ���µ�vtkPolyData����
  inputPolyData->SetLines(inputCellArray);                                          // ������������
  inputPolyData->SetPoints(inputPoints);                                            // ������������

  // ʹ�� vtkDelaunay2D ���ж�ά���ǻ�
  vtkSmartPointer<vtkDelaunay2D> delaunay = vtkSmartPointer<vtkDelaunay2D>::New();
  delaunay->SetInputData(inputPolyData);
  delaunay->SetAlpha(delaunayAlpha);
  delaunay->Update();
  // vtkSmartPointer< vtkDelaunay3D > delaunay = vtkSmartPointer< vtkDelaunay3D >::New();// ����һ���µ�Delaunay3D����
  // delaunay->SetAlpha(delaunayAlpha);// ����alphaֵ
  // delaunay->AlphaTrisOff();// �ر�alpha������
  // delaunay->AlphaLinesOff();// �ر�alpha����
  // delaunay->AlphaVertsOff();// �ر�alpha����

  vtkSmartPointer<vtkMatrix4x4> boundingAxesToRasTransformMatrix = vtkSmartPointer<vtkMatrix4x4>::New(); // ����һ���µ�4x4����
  ComputeTransformMatrixFromBoundingAxes(inputPoints, boundingAxesToRasTransformMatrix);                 // ����ӱ߽��ᵽRAS�ı任����

  vtkSmartPointer<vtkMatrix4x4> rasToBoundingAxesTransformMatrix = vtkSmartPointer<vtkMatrix4x4>::New(); // ����һ���µ�4x4����
  vtkMatrix4x4::Invert(boundingAxesToRasTransformMatrix, rasToBoundingAxesTransformMatrix);              // ����任����������

  double smallestBoundingExtentRanges[3] = {0.0, 0.0, 0.0};                                                    // temporary values // ��ʼ��һ����������Ԫ�ص����飬���ڴ洢��С�߽緶Χ
  ComputeTransformedExtentRanges(inputPoints, rasToBoundingAxesTransformMatrix, smallestBoundingExtentRanges); // ����任��ķ�Χ

  PointArrangement pointArrangement = ComputePointArrangement(smallestBoundingExtentRanges); // �������������

  switch (pointArrangement) // ���ݵ���������ѡ������ʽ
  {
  case POINT_ARRANGEMENT_NONPLANAR: // ��ƽ���������--- �����Ķ������õĴ���
  {
    delaunay->SetInputData(inputPolyData); // ����Delaunay����������
    break;
  }
  default: // unsupported or invalid // ��֧�ֻ���Ч���������
  {
  ...
  vtkSmartPointer<vtkDataSetSurfaceFilter> surfaceFilter = vtkSmartPointer<vtkDataSetSurfaceFilter>::New(); // ����һ���µ�DataSetSurfaceFilter����
  surfaceFilter->SetInputConnection(delaunay->GetOutputPort());                                             // ������������
  surfaceFilter->Update();                                                                                  // ��������

  vtkSmartPointer<vtkPolyDataNormals> normals = vtkSmartPointer<vtkPolyDataNormals>::New(); // ����һ���µ�PolyDataNormals����
  // normals->SetFeatureAngle(0); //  ԭ����100 TODO: This needs some justification, or set as an input parameter // ����������
  normals->SetFeatureAngle(180); // ����������Ϊ180����ȷ�����߼��㲻��պ�����

  vtkGenericWarningMacro("1.1smoothing:" << smoothing << ",  forceConvex:" << forceConvex);

  if (smoothing) // �����Ҫƽ���������ҵ���������Ϊ��ƽ��
  {              // ����һ���µ�ButterflySubdivisionFilter����
    ...
    subdivisionFilter->SetNumberOfSubdivisions(3);                         // ����ϸ�ִ���
    subdivisionFilter->Update();                                           // �������ݣ������surfaceFilter����Ϣ
                                                                           // if (forceConvex)                                                       // �����Ҫǿ��͹��
                                                                           // {
    // vtkSmartPointer<vtkDelaunay3D> convexHull = vtkSmartPointer<vtkDelaunay3D>::New();                        // ����һ���µ�Delaunay3D����
  ...
  {
    vtkNew<vtkLinearSubdivisionFilter> linearSubdivision;                  // ����һ���µ�LinearSubdivisionFilter����
