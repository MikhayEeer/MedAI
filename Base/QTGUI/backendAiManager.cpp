...
void backendAiManager::showFileDialog(){
    this->show();
...
void backendAiManager::uploadFile(QStringList files) {
    ...
    //QNetworkRequest request(QUrl("http://118.195.135.211:7998/upload")); //119.45.186.16 // 127.0.0.1
    
    QNetworkRequest request(QUrl("http://36.137.227.17:11143/upload"));
...
void backendAiManager::initUI(){
    ...
    this->setWindowTitle(tr("动静脉自动分割窗口"));

    //用户名Label
    userNameLbl = new QLabel(this);
    userNameLbl->setText("请注意:");
    //密码Label
    pwdLbl = new QLabel(this);
    pwdLbl->setText("处理时间在3分钟~10分钟不等，取决于上传的图像质量");
